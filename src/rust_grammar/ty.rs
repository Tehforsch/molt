...
use super::{
    Attribute, Expr, Ident, Lifetime, LitStr, Macro, TraitBound, TraitBoundModifier,
    TypeParamBound,
    path::{Path, PathArguments, QSelf},
    prelude::*,
};
...
pub(crate) fn ambig_ty(
    ...
    if input.peek(token::Group) {
        ...
        if input.peek(Token![::]) && input.peek3_ident() {
    ...
    if lookahead.peek(token::Paren) {
        let content;
        let syn_content;
        let paren_token = parenthesized!(syn_content in content in input);
        if content.is_empty() {
        ...
        || lookahead.peek(Token![<])
    {
        ...
        let content;
        let syn_content;
        let bracket_token = bracketed!(syn_content in content in input);
...
impl Parse for TypeSlice {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        let syn_content;
        Ok(TypeSlice {
            bracket_token: bracketed!(syn_content in content in input),
            elem: content.parse()?,
        })
    }
}
...
impl Parse for TypeBareFn {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        Ok(TypeBareFn {
            ...
            paren_token: parenthesized!(syn_args in args in input),
            inputs: {
...
impl Parse for TypePath {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        let (qself, path) = super::path::qpath(input, expr_style)?;
...
impl Parse for TypeGroup {
    fn parse(input: ParseStream) -> Result<Self> {
        todo!()
        // let group = super::group::parse_group(input)?;
        // Ok(TypeGroup {
...
impl TypeParen {
    fn parse(input: ParseStream, allow_plus: bool) -> Result<Self> {
        ...
        let syn_content;
        Ok(TypeParen {
            paren_token: parenthesized!(syn_content in content in input),
            elem: Box::new({
                let allow_group_generic = true;
