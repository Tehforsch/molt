...
mod value {
    ...
    use crate::rust_grammar::bigint::BigInt;
    use crate::rust_grammar::lit::{
    ...
    pub(crate) fn parse_lit_int(mut s: &str) -> Option<(Box<str>, Box<str>)> {
                ...
                b'e' | b'E' if base == 10 => {
                    ...
                    for (i, b) in s[1..].bytes().enumerate() {
                        match b {
                            ...
                            _ => {
                                let suffix = &s[1 + i..];
                                if has_exp && crate::rust_grammar::ident::xid_ok(suffix) {
                                    return None;
                                } else {
                                    break 'outer;
                                }
        ...
        let suffix = s;
        if suffix.is_empty() || crate::rust_grammar::ident::xid_ok(suffix) {
