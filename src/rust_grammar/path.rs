use super::{ExprBlock, ExprPath, prelude::*, ty::Type};
use crate::ctx::NodeId;

use super::{Expr, Ident, Lifetime, ReturnType, TypeParamBound};
...
impl Parse for GenericArgument {
    fn parse(input: ParseStream) -> Result<Self> {
        if input.peek_lifetime() && !input.peek2(Token![+]) {
            return Ok(GenericArgument::Lifetime(input.parse()?));
        }

        if input.peek_lit() || input.peek(token::Brace) {
        ...
        match argument {
                    ...
                    } =>
            {
                if let Some(eq_token) = input.parse::<Option<Token![=]>>()? {
                    ...
                    return if input.peek_lit() || input.peek(token::Brace) {
...
impl Parse for ParenthesizedGenericArguments {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        Ok(ParenthesizedGenericArguments {
            ...
            inputs: content.parse_terminated_internal(Type::parse, Token![,])?,
            output: input.call_internal(ReturnType::without_plus)?,
        })
    }
}
